#ifndef IHT_BGHT_WRAPPER
#define IHT_BGHT_WRAPPER
...
namespace wrappers {
   ...
   struct iht_wrapper {
      ...
      using internal_table_type = bght::iht_generic<Key, Val, bucket_size>;

      using my_type = iht_wrapper<Key, Val, bucket_size>;
      ...
      static __host__ my_type * generate_on_device(uint64_t cache_capacity, Key sentinel_key, Key tombstone_key, Val sentinel_val){
         ...
         host_version->internal_table = internal_table_type::generate_on_device(cache_capacity, sentinel_key, sentinel_val);
      ...
      __device__ bool insert_if_not_exists(tile_type my_tile, Key key, Val val){
         return internal_table->insert(pack_together(key, val), my_tile).second;
      }
      ...
      static char * get_name(){
         return "Icerberg Hashing (Double hash)";
      }
