...
namespace tables {
   ...
   struct p2_ext_table {
      ...
      static __host__ my_type * generate_on_device(uint64_t cache_capacity, uint64_t ext_seed){
         ...
         host_version->n_buckets_primary = ext_n_buckets*.9;
         host_version->n_buckets_alt = ext_n_buckets*.1;
         ...
         init_iht_table_kernel<my_type><<<(host_version->n_buckets_primary-1)/256+1,256>>>(device_version);
